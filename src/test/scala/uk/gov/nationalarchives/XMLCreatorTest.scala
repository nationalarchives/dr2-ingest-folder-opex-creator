...
class XMLCreatorTest extends AnyFlatSpec {
  ...
  val expectedXml: Elem = <opex:OPEXMetadata xmlns:opex="http://www.openpreservationexchange.org/opex/v1.0">
    <opex:Properties>
    ...
    <opex:Transfer>
      ...
      <opex:Manifest>
        <opex:Folders>
          ...
          <opex:Folder>name0 Folder</opex:Folder>
          <opex:Folder>name1 Folder</opex:Folder>
        </opex:Folders>
        <opex:Files>
  ...
  "createOpex" should "create the correct opex xml" in {
    ...
    prettyPrinter.format(expectedXml) should equal(xml)
  }
}
